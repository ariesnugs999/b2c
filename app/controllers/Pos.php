...
class Pos extends MY_Controller {
    ...
    function close_register($user_id = NULL) {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['totalpurchase'] = $this->pos_model->getPurchase();
        ...
        if($products) {
            if($this->Settings->bsty == 1) {
                ...
                foreach($products as $product) {
                    ...
                    if($count < 10) { $count = "0".($count /100) *100;  }
                    if($category_id < 10) { $category_id = "0".($category_id /100) *100;  }
                    $prods .= "<button type=\"button\" data-name=\"".$product->name."\" id=\"product-".$category_id.$count."\" type=\"button\" value='".$product->code."' class=\"btn btn-img btn-flat product\"><img src=\"".base_url()."uploads/thumbs/".$product->image."\" alt=\"".$product->name."\" style=\"width: 110px;height: 110px;\"><div style=\"width:110px; height:auto; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;\">".$product->name."</div></button>";
                    $pro++;
                }
