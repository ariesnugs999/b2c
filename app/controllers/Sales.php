...
class Sales extends MY_Controller {
    ...
    function get_sales() {
        ...
        if ($this->db->dbdriver == 'sqlite3') {
            $this->datatables->select("id, strftime('%Y-%m-%d %H:%M', date) as date, customer_name, total, total_tax, total_discount, grand_total, paid, (grand_total-paid) as balance, status");
        } else {
    ...
    function receivable() {
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('list_receivable');
        $bc = array(array('link' => '#', 'page' => lang('list_receivable')));
        $meta = array('page_title' => lang('list_receivable'), 'bc' => $bc);
        $this->page_construct('sales/receivable', $this->data, $meta);
    }
