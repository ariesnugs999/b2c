...
class Reports_model extends CI_Model
{
    ...
    public function getTotalDailySales() {
        $this->db->select('count(id) as total, sum(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid,(sum(COALESCE(grand_total, 0)) -  SUM(COALESCE(paid, 0))) as piutang, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
            ->where("DATE(date) = DATE(NOW())", NULL, FALSE);
        if ($this->session->userdata('store_id')) {
            $this->db->where('store_id', $this->session->userdata('store_id'));
        }
        $q = $this->db->get('sales');
    ...
    public function getTotalDailyPurchases() {
        $this->db->select('count(id) as total, sum(COALESCE(total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid,(sum(COALESCE(total, 0)) -  SUM(COALESCE(paid, 0))) as hutang, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
            ->where("DATE(date) = DATE(NOW())", NULL, FALSE);
        if ($this->session->userdata('store_id')) {
            $this->db->where('store_id', $this->session->userdata('store_id'));
        }
    ...
    public function getTotalDailyExpenses() {
    ...
    // Added by Bayu
    public function getTotalSupplierPurchases($supplier_id, $user = NULL, $start_date = NULL, $end_date = NULL) {
        $this->db->select('COUNT(id) as number, sum(grand_total) as amount, sum(paid) as paid');
        if ($start_date && $end_date) {
            $this->db->where('date >=', $start_date);
            $this->db->where('date <=', $end_date);
        ...
        $q = $this->db->get_where('purchases', array('supplier_id' => $supplier_id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    // Added by Bayu
    public function viewReportProfitProducts($dateStart,$dateEnd){
        $dataSelect = array(
            "pro.id","pro.name","pro.code","psq.quantity as sisa", "sum(pi.subtotal) as total_pembelian", 
            "coalesce(sum(purchase_items.subtotal) / sum(purchase_items.quantity),0) AS avg_purchase_price", 
            "sum(pi.quantity) as purchase"
        );

        $this->db->select($dataSelect);
        // $this->db->from("products pro");
        $this->db->join("product_store_qty psq","product_store_qty.product_id=products.id", "left");
        $this->db->join('purchase_items pi', 'purchase_items.product_id=product_store_qty.product_id', 'left');
        $this->db->join('purchases pur', 'purchases.id=purchase_items.purchase_id', 'left');

        if ($this->session->userdata('store_id')) {
            $this->db->where('psq.store_id', $this->session->userdata('store_id'));
        }

        if(!empty($dateStart) && !empty($dateEnd)){
            $this->db->where("pur.date BETWEEN '{$dateStart}' AND '{$dateEnd}'");
        }
