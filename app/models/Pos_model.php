...
class Pos_model extends CI_Model
{
            ...
            if(! empty($payment)) {
                if ($payment['paid_by'] == 'stripe') {
                    ...
                    if (!isset($result['error']) && !empty($result['transaction_id'])) {
                        ...
                        $this->db->update('sales', ['paid' => 0, 'status' => 'unpaid'], ['id' => $sale_id]);
    ...
    public function updateSale($id, $data, $items) {
        ...
        $data['status'] = $osale->paid > 0 ? 'partial' : ($data['grand_total'] <= $osale->paid ? 'paid' : 'unpaid');
    ...
    public function getPurchase() {
        $this->db->select('SUM( COALESCE( total, 0 ) ) AS total, SUM( COALESCE( paid, 0 ) ) AS paid', FALSE);
